...
def get_summary(
    ...
    income_sum = db.query(func.sum(Transaction.amount)).filter(Transaction.type == "receita").scalar() or 0.0
    expense_sum = db.query(func.sum(Transaction.amount)).filter(Transaction.type == "despesa").scalar() or 0.0
    ...
    recent_txs = (
        ...
        .filter(Transaction.date_time >= start_recent, Transaction.date_time <= end_recent)
